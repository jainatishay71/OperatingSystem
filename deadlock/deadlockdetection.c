...
int main()
{
	...
	for(int i=0;i<n;i++)
	{
		for(int j=0;j<m;j++)
		{
			...
			{
				allocation[i][j]=0;
				request[i][j]=maxi[i][j];
			}
			else
			{
				allocation[i][j]=rand()%(maxi[i][j]);
				available[j]-=allocation[i][j];
				request[i][j]=maxi[i][j]-allocation[i][j];
			}
		}
	}
	printf("\n");
	printf("Allocated\tMaximum\t\tRequest\n");
	for(int i=0;i<n;i++)
	{
		for(int j=0;j<m;j++)
		{
	...
	while(1)
	{
		...
		if(flag==0)
		{
			printf("Its safer to grant requests as it won't lead to Deadlock\n");
			break;
		}
		if(cnt>1000)
		{
